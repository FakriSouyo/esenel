    ...
    >
      <div className="relative">
        <div
          ...
          className="cursor-grab overflow-hidden py-10 outline-none focus-visible:ring-2 focus-visible:ring-ink/20 active:cursor-grabbing"
          style={{
          ...
          >
            {slides.map((slide, index) => (
              <div
                ...
                className={cn(
                  'absolute left-1/2 top-0 aspect-square overflow-hidden rounded-2xl bg-sand/30 shadow-xl will-change-transform',
                  cardClassName,
                )}
        ...
        {showNavigation && (
          <>
            <button
              ...
              onClick={() => nudge(-1)}
              className="absolute left-3 top-1/2 z-[200] -translate-y-1/2 rounded-full bg-white/70 p-2 text-ink shadow-md backdrop-blur transition hover:bg-white"
            >
            ...
            <button
              ...
              onClick={() => nudge(1)}
              className="absolute right-3 top-1/2 z-[200] -translate-y-1/2 rounded-full bg-white/70 p-2 text-ink shadow-md backdrop-blur transition hover:bg-white"
            >
      ...
      {showCaption && active?.title && (
        <div key={selected} className="mt-4 flex flex-col items-center px-6">
          <p className="font-display text-2xl md:text-3xl leading-tight text-ink">
            {active.title}
          </p>
          {active.subtitle && (
            <p className="mt-1 text-[13px] text-ink/55">{active.subtitle}</p>
          )}
          {active.meta && active.meta.length > 0 && (
            <dl className="mt-6 w-full max-w-[230px] text-[12px]">
              {active.meta.map((row) => (
                <div
                  key={row.label}
                  className="flex justify-between border-b border-ink/10 py-[5px] last:border-0"
                >
        ...
        <div className="mt-6 flex items-center justify-center gap-2">
          {slides.map((_, index) => (
            <button
              ...
              onClick={() => goTo(index)}
              className={cn(
                'size-2 rounded-full bg-ink transition-opacity',
