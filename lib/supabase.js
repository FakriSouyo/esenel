...
/**
 * ---- Craft builder gallery (submitted bouquets) ----
 */

/**
 * Upload a bouquet PNG (dataURL from the Konva stage) to the public
 * `bouquet-gallery` bucket and return its public URL.
 */
export async function uploadBouquetImage(dataUrl) {
  if (!SUPABASE_URL || !SUPABASE_ANON_KEY || typeof dataUrl !== 'string') {
    throw new Error('Missing Supabase env — copy .env.example to .env.local.');
  }
  const mime = String(dataUrl).match(/^data:(image\/[a-zA-Z+]+);base64,/);
  ...
  const key = `${crypto.randomUUID()}.${contentType.includes('png') ? 'png' : 'jpg'}`;
  const res = await fetch(`${SUPABASE_URL}/storage/v1/object/bouquet-gallery/${key}`, {
    method: 'POST',
    headers: {
      apikey: SUPABASE_ANON_KEY,
      Authorization: `Bearer ${SUPABASE_ANON_KEY}`,
  ...
  return storagePublicUrl('bouquet-gallery', key);
}

/**
 * Persist a saved bouquet to the gallery (shared with all visitors).
 */
export async function saveSubmittedBouquet(payload) {
  await supabaseFetch('/submitted_bouquets', {
    method: 'POST',
    body: {
      bouquet_name: payload.bouquet_name,
      maker_name: payload.maker_name ?? null,
      image_url: payload.image_url ?? null,
      size: payload.size ?? null,
      wrapping: payload.wrapping ?? null,
...
/**
 * Fetch the most recent published bouquets for the community gallery.
 */
export async function fetchSubmittedBouquets({ limit = 24 } = {}) {
  if (!SUPABASE_URL || !SUPABASE_ANON_KEY) return [];
  const res = await supabaseFetch(
    `/submitted_bouquets?select=id,bouquet_name,maker_name,image_url,size,wrapping,flowers,created_at&order=created_at.desc&limit=${limit}`
  );
  const rows = await res.json();
