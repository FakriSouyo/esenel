...
/**
 * Versi gambar OG — URL OG memakai `?v=` supaya saat desain OG diubah,
 * URL-nya ikut berubah dan cache CDN Vercel (immutable max-age 1 tahun)
 * tidak menyajikan gambar lama. Naikkan angka ini setiap kali OG diubah.
 */
export const OG_VERSION = 2;

/** Objek gambar untuk metadata openGraph — path relatif, di-resolve ke
 *  absolut oleh metadataBase di root layout. URL memuat versi (cache-bust). */
export function ogImage(page) {
  const cfg = OG_PAGES[page] || OG_PAGES.home;
  return {
    url: `/og?page=${page}&v=${OG_VERSION}`,
    width: 1200,
    height: 630,
