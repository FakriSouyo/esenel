...
/** Five full-width vertical strips (ramping) — widened so it reads as a
 *  proper slim cone, not a few thin needles. */
export function frontSlim(g) {
  const { cx, tieY, halfW, coneH } = g;
  const n = 5;
  const spread = halfW * 0.82;
  const panels = [];
  for (let i = 0; i < n; i++) {
    const t = (i / (n - 1)) * 2 - 1;
    const px = cx + t * spread;
    const py = tieY - coneH * (0.82 + 0.22 * (1 - Math.abs(t)));
    const baseHalf = halfW * 0.11;
    const d = [
      `M ${(cx + t * halfW * 0.16).toFixed(1)} ${tieY.toFixed(1)}`,
      `L ${(px - baseHalf).toFixed(1)} ${(tieY - coneH * 0.32).toFixed(1)}`,
      `L ${px.toFixed(1)} ${py.toFixed(1)}`,
      `L ${(px + baseHalf).toFixed(1)} ${(tieY - coneH * 0.32).toFixed(1)} Z`,
    ].join(' ');
