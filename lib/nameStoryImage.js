...
export const NO_LIVING_BEINGS_SUFFIX =
  'Bouquet stands alone, NOT held by anyone, no people, no human hands, no animals, no insects, no living beings.';

/** Larangan vas/wadah — generator gambar sering menaruh bunga di vas kaca
 *  padahal yang dimau adalah buket tangan yang dibungkus kertas. */
export const NO_VASE_SUFFIX =
  'Hand-tied bouquet wrapped in kraft paper with visible stems, NOT in a vase, no glass vase, no ceramic vase, no pot, no bottle, no water, no container.';

/**
 * Pastikan imagePrompt: (1) menyebut SEMUA bunga dari daftar kecocokan satu
 * per satu dalam Bahasa Inggris (biar tidak monoton), (2) dikunci sebagai
 * buket TANGAN yang DIBUNGKUS kertas (bukan vas), (3) difoto ala foto produk
 * katalog ESENEL — kertas kraft, latar krem studio, cahaya lembut, dan
 * (4) foto STILL LIFE — buket berdiri sendiri, bukan dipegang orang.
 *
 * Teruji live (Cloudflare Worker utama, 13s/gen): prompt PANJANG bergaya
 * "catalog photo" yang menumpuk banyak deskripsi membuat model worker
 * ambruk ke SATU variasi dominan (terbukti 46% merah marun, tanpa biru/
 * kuning/putih). Format PENDEK di bawah — jumlah bunga eksplisit + warna
 * tiap bunga di depan + still life ringkas — menghasilkan buket CAMPURAN
 * dengan banyak variasi terlihat jelas (biru, kuning, putih, pink hadir).
 */
export function enforceImagePrompt(story) {
  ...
  story.imagePrompt =
    `A single bouquet mixing ${flowers.length} different flowers side by side, all clearly visible and distinct: ${flowers.join(', ')}. ` +
    `Photographed like a premium floral catalog product: hand-tied dome-shaped bouquet wrapped in matte kraft paper with neat visible stems, standing on a plain wooden table, warm cream studio background, soft diffused natural light, front-facing, centered, breathing room around the bouquet, photorealistic. ` +
    NO_VASE_SUFFIX + ' ' +
