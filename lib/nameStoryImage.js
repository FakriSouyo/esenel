...
export function enforceImagePrompt(story) {
  ...
  // Buket hand-tied TUNGGAL yang berisi HANYA bunga-bunga pilihan itu, gaya
  // foto katalog + mengikuti foto acuan. Dilarang keras orang/tangan/objek/
  // vas/teks, dan dilarang keras menambahkan bunga di luar daftar.
  story.imagePrompt =
    `One hand-tied dome-shaped bouquet containing ONLY these flowers and no others, every one clearly visible and distinct: ${flowerList}. ` +
    `Wrapped in matte kraft paper with neat visible stems, tied with natural jute twine, standing upright on a plain light wooden table, warm cream studio backdrop, soft diffused window light, front-facing, centered, breathing room around the bouquet. ` +
    `Match the composition, color palette and photographic style of the reference bouquet photos provided. ` +
    `Photorealistic premium floral catalog product photography. NO people, NO hands, NO faces, NO animals, NO insects, NO vase, NO pot, NO bottle, NO water, NO gifts, NO text, NO logo, NO watermark — the bouquet stands alone on the table, and no flowers other than the ones listed.`;
...
export async function generateWithBitdeer(prompt, { timeoutMs = 120000, referenceImages = [] } = {}) {
  ...
  try {
    ...
    for (let i = 0; i < candidatesList.length; i++) {
      ...
      // Sniff the ACTUAL image bytes — the response content-type header is
      // usually "application/json" (even though the payload is base64), so
      // using it directly would store the image as a broken .json blob.
      // Always detect PNG/JPEG from the decoded magic bytes instead.
      const realType = sniffContentType(parsed.buffer);
      const elapsed = ((Date.now() - startTime) / 1000).toFixed(1);
      console.log(`[name-image] Bitdeer: image ready (${elapsed}s, ${parsed.buffer.length} bytes, ${realType})`);
      return { buffer: parsed.buffer, contentType: realType };
    }
